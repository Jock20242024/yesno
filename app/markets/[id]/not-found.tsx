...
export default function NotFound() {
  ...
  const [text, setText] = useState({ message: "Market event not found", button: "Back to Home" });
  ...
  useEffect(() => {
    ...
    // 立即更新文本
    if (currentLang === "zh") {
      setText({ message: "市场事件未找到", button: "返回首页" });
    } else {
      setText({ message: "Market event not found", button: "Back to Home" });
    }
    
    // 然后使用翻译函数更新（确保使用最新的翻译）
    const marketNotFound = t("common.not_found.market_not_found");
    const backToHome = t("common.not_found.back_to_home");
    
    // 只有当翻译函数返回的值不是 key 本身时才使用（说明翻译成功）
    if (marketNotFound !== "common.not_found.market_not_found") {
      setText({ message: marketNotFound, button: backToHome });
    }
  }, [t]);
