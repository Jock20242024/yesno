/**
 * 分类初始化脚本
 * 将前端导航栏现有的分类（crypto, politics, sports, finance, tech）插入到数据库的 Category 表中
 * 
 * 运行方式: npx ts-node scripts/seed-categories.ts
 */
...
const CATEGORIES_TO_SEED = [
  { slug: 'crypto', name: '加密货币', displayOrder: 1 },
  { slug: 'politics', name: '政治', displayOrder: 2 },
  { slug: 'sports', name: '体育', displayOrder: 3 },
  { slug: 'finance', name: '金融', displayOrder: 4 },
  { slug: 'tech', name: '科技', displayOrder: 5 },
];
...
async function main() {
  ...
  console.log(`   总计: ${CATEGORIES_TO_SEED.length} 个分类\n`);
