...
**修复内容**：
- ✅ 添加 `PositionStatus` 枚举（OPEN | CLOSED）
- ✅ 添加 `Position` 模型（包含status字段）
- ✅ 在 `Order` 模型中添加 `type` 字段（BUY | SELL）
...
**修复内容**：
- ✅ 从Position表查询OPEN状态的持仓（不再从Order数组计算）
- ✅ 计算持仓价值（基于当前市场价格）
...
```bash
# 1. 更新Prisma Schema（已完成）
# prisma/schema.prisma 已添加Position表和PositionStatus枚举
...
1. ✅ **修复买入API**：`app/api/orders/route.ts` - 已添加Position创建逻辑
2. ✅ **创建卖出API**：`app/api/orders/sell/route.ts` - 已创建完整实现
3. ✅ **创建持仓查询API**：`app/api/positions/route.ts` - 已创建完整实现
4. ✅ **修复资产汇总API**：`app/api/user/assets/route.ts` - 已更新使用Position表
5. ✅ **修复市场详情页**：`app/api/markets/[market_id]/route.ts` - 已更新使用Position表

### 步骤3：修复前端（P1 - 待完成）

1. ⚠️ **修复WalletPage**：移除硬编码，使用API数据
2. ⚠️ **修复TradeSidebar**：添加status检查
3. ⚠️ **修复StoreContext**：移除前端模拟交易
...
- [x] `/api/orders` (BUY) 创建Position记录
- [x] `/api/orders/sell` (SELL) 更新Position.status = CLOSED
- [x] `/api/positions` 只返回OPEN状态的持仓
- [x] `/api/user/assets` 从Position表计算持仓价值
- [x] `/api/markets/[market_id]` 从Position表查询持仓
...
1. ✅ `prisma/schema.prisma` - 添加Position表和PositionStatus枚举
2. ✅ `app/api/orders/route.ts` - 修复BUY API创建Position
3. ✅ `app/api/orders/sell/route.ts` - 新建SELL API
4. ✅ `app/api/positions/route.ts` - 新建持仓查询API
5. ✅ `app/api/user/assets/route.ts` - 修复使用Position表
6. ✅ `app/api/markets/[market_id]/route.ts` - 修复使用Position表
7. ✅ `prisma/migrations/add_position_table.sql` - 数据库迁移SQL

### ⚠️ 待完成

8. ⚠️ `app/wallet/page.tsx` - 移除硬编码，使用API数据
9. ⚠️ `components/market-detail/TradeSidebar.tsx` - 添加status检查
10. ⚠️ `app/context/StoreContext.tsx` - 移除前端模拟交易
...
- 已完成，见第四部分
- 关键代码文件已创建：
  - `app/api/orders/sell/route.ts` - SELL API完整实现
  - `app/api/positions/route.ts` - 持仓查询API完整实现
  - `prisma/schema.prisma` - Position表定义
  - `prisma/migrations/add_position_table.sql` - 数据库迁移SQL
